...
import { getAvailableSlots, dayOfWeekFromDateStr, toLocalTimeStr } from "@/lib/availability";
...
export async function GET(req: NextRequest) {
  ...
  if (!restaurantId || !date) return NextResponse.json({ error: "Missing restaurantId or date" }, { status: 400 });
  ...
  const [hours, blackouts, tables, reservations, sessions] = await Promise.all([
    prisma.openingHour.findUnique({ where: { restaurantId_dayOfWeek: { restaurantId, dayOfWeek: dow } } }),
    prisma.blackoutDate.findMany({ where: { restaurantId } }),
    prisma.diningTable.findMany({ where: { restaurantId } }),
    ...
    prisma.tableSession.findMany({ where: { restaurantId, status: "SEATED" } }),
  ]);
  ...
  return NextResponse.json({
    date,
    dayOfWeek: dow,
    settings: { bookingWindowDays: (await prisma.restaurantSettings.findUnique({ where: { restaurantId } }))?.bookingWindowDays ?? null },
    ...
    reservationsTouchingThisDate: reservations
      .filter((r) => r.reservationTime.toISOString().slice(0, 10) === date)
      .map((r) => ({
        id: r.id,
